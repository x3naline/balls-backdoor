...
export default {
  ...
  async getProfile(req, res, next) {
    try {
      ...
      if (!user) {
        return res.status(404).json(responseFormatter.error("User not found", "NOT_FOUND"))
      }

      return res.status(200).json(responseFormatter.success("Profile retrieved successfully", user))
    } catch (error) {
      next(error)
    }
  },
  ...
  async updateProfile(req, res, next) {
    try {
      ...
      return res.status(200).json(responseFormatter.success("Profile updated successfully"))
    } catch (error) {
      next(error)
    }
  },
